import fs from 'fs';
import path from 'path';
import { fetchPostByIdServer } from '../server/postService';
import { injectPostMetadataIntoHtml } from '../server/htmlInjector';

export default async function handler(req: any, res: any) {
  let postId =
    (req.query?.id as string) ||
    (req.query?.postId as string) ||
    (req.query?.prompt as string) ||
    (req.query?.post as string) ||
    (req.query?.p as string) ||
    '';

  if (!postId && req.url) {
    const match = req.url.match(/\/post\/([^?#/]+)/i);
    if (match && match[1]) {
      postId = decodeURIComponent(match[1]);
    }
  }

  try {
    // Look for index.html in standard build / source locations
    const candidatePaths = [
      path.join(process.cwd(), 'dist', 'index.html'),
      path.resolve(__dirname, '../dist/index.html'),
      path.resolve(__dirname, '../../dist/index.html'),
      path.join(process.cwd(), 'index.html'),
      path.resolve(__dirname, '../index.html'),
    ];

    let rawHtml = '';
    for (const candidate of candidatePaths) {
      if (fs.existsSync(candidate)) {
        try {
          rawHtml = fs.readFileSync(candidate, 'utf-8');
          if (rawHtml) break;
        } catch (e) {
          // continue checking
        }
      }
    }

    if (!rawHtml) {
      rawHtml = `<!doctype html>
<html lang="en" class="dark">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Sahil Edits – Premium AI Prompt Library</title>
  <meta name="description" content="Sahil Edits – The ultimate curated AI Prompt Library for ChatGPT, Gemini, Claude, Midjourney, and Flux." />
  <link rel="canonical" href="https://sahiledit.vercel.app/" id="seo-canonical-link" />
  <meta property="og:type" content="website" id="seo-og-type" />
  <meta property="og:url" content="https://sahiledit.vercel.app/" id="seo-og-url" />
  <meta property="og:title" content="Sahil Edits – Premium AI Prompt Library" id="seo-og-title" />
  <meta property="og:description" content="Sahil Edits – The ultimate curated AI Prompt Library for ChatGPT, Gemini, Claude, Midjourney, and Flux." id="seo-og-description" />
  <meta property="og:image" content="https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=1200&q=80" id="seo-og-image" />
  <meta property="og:site_name" content="Sahil Edits" />
  <meta name="twitter:card" content="summary_large_image" id="seo-twitter-card" />
  <meta name="twitter:url" content="https://sahiledit.vercel.app/" id="seo-twitter-url" />
  <meta name="twitter:title" content="Sahil Edits – Premium AI Prompt Library" id="seo-twitter-title" />
  <meta name="twitter:description" content="Sahil Edits – The ultimate curated AI Prompt Library for ChatGPT, Gemini, Claude, Midjourney, and Flux." id="seo-twitter-description" />
  <meta name="twitter:image" content="https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=1200&q=80" id="seo-twitter-image" />
</head>
<body class="bg-[#0F172A] text-zinc-100">
  <div id="root"></div>
</body>
</html>`;
    }

    const post = postId ? await fetchPostByIdServer(postId) : null;
    const finalHtml = injectPostMetadataIntoHtml(rawHtml, post, req, postId);

    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Cache-Control', 'public, max-age=60, s-maxage=300, stale-while-revalidate=86400');
    return res.status(200).send(finalHtml);
  } catch (error: any) {
    console.error('[Vercel Post OG Handler Error]', error);
    return res.status(500).send('Internal Server Error');
  }
}

